...
public class VisitController {
    ...
    public VisitDto loadPetWithVisit(@PathVariable("petId") @Min(0) Long petId,
        ...
        visit.setPet(pet);
        return new DTOConverter().toVisitDto(visit);
    }

    @GetMapping("/owners/*/pets/{petId}/visits/new")
    public String initNewVisitForm(@PathVariable("petId") @Min(0) Long petId,
                                   Map<String, Object> model) {
    ...
    public String processNewVisitForm(VisitDto visitDto, BindingResult result) {
        Visit visit = new DTOConverter().toVisit(visitDto);
        log.info("Try to create visit ...");
        if (result.hasErrors()) {
